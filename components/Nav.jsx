...
const Links = [
  ...
  },
  {
    ...
    title: "Services",
    icon: (
      <MdOutlineMiscellaneousServices className="md:hidden white dark:black" />
    ),
  },
  {
    link: "/skills",
  ...
  },
  {
    ...
    title: "Portfolio",
    icon: (
      <AiOutlineFundProjectionScreen className="md:hidden white dark:black" />
    ),
  },
  {
    link: "/contacts",
        ...
        {Links.map(({ link, title, icon }) => (
          <li key={title}>
            <Link href={link}>
              <a
                className={`text-3xl ${
                  router.pathname === link ? "text-main-color" : null
                }`}
              >
